...
- [interview/](interview/) — five persona scripts ([01-qa-engineer](interview/01-qa-engineer-interview.md), [02-qa-lead](interview/02-qa-lead-interview.md), [03-developer](interview/03-developer-interview.md), [04-release-manager](interview/04-release-manager-interview.md), [05-product-owner](interview/05-product-owner-interview.md)) plus [00-interview-guide-overview.md](interview/00-interview-guide-overview.md) which defines methodology, sequence, and the question-tagging convention. The form generator reads them; if you edit, preserve the tag taxonomy and keep question numbering contiguous within each script.
...
- **Section tags on questions** — every interview question is tagged with the brief section it feeds: `[1]`, `[3a]`, `[3b]`, `[3c]`, `[3d]`, `[4a]`, `[5]`, `[6-A]`, `[6-R]`, `[6-D]`, `[6-S]`, `[7]`, `[8]`. Preserve tags verbatim on any edit; they drive both the form output and the future compiler agent. Untagged questions are warm-ups.
- **No `[4b]` interview tag** — the brief's §4b AI Integration Points is filled by the author/architect from external AI tooling assessment, not from interview synthesis. Participants have varying levels of AI knowledge, so probing them on AI fit produces noise rather than signal. Discovery interviews extract problems and pain points only.
...
- **Solution-agnostic framing** — neither the brief nor the scripts should name vendors, tools, or specific AI capabilities during discovery. The brief is a decision document, not a sales pitch.
- **No leading questions, no solutioning during discovery** — applies in the script wording.
- **Symmetric risks** — roughly half of `[6-R]` should describe failure modes of the proposed change, not only current-state pain. Phrase failure-mode probes in neutral "new tooling" / "automated verification" language; never name AI in the question.
- **`[6-S]` Security & Data Governance** is sourced from the QA Lead script; flag as empty if the Lead skipped it.

## Out of scope

- Adding `[4b]` AI-fit, AI-capability, or "where would AI help here?" questions to any script. Removed deliberately: the brief's §4b is author/architect-sourced from external capability assessment, not from interviews.
